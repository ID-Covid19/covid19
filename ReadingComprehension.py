...
def answer_from_full_text(full_text_question_idx_pair):  
    ...
    if (str(full_text_question_idx_pair)) in processed_index:
        ...
        current_answer = predictor.predict(questions[question_idx],full_texts[full_text_idx])
        
        # Save the answer as a dataframe
        answers = pd.DataFrame.from_dict([{"full_text":full_texts[full_text_idx],
...
if __name__ == "__main__":
    ...
    results = ThreadPool(3).imap_unordered(answer_from_full_text, full_text_and_question_pairs) # TODO: remove index
